...
class AppBootHook {
  ...
  async didLoad() {
    ...
    assert(
      !!client.namespace,
      "[eggjs-nacos] Property ‘namespace’ is required!"
    );
    ...
    this.app.messenger.once("egg-server-realport", async (port) => {
      ...
      if (isRegisterInstance) {
        ...
        // 注销注册时使用
        Object.assign(client, { serviceName, ip, port, groupName, metadata });
        ...
        this.app.logger.info(
          `[eggjs-nacos] 配置 isRegisterInstance: ${isRegisterInstance} ，已关闭默认注册实例`
        );
      }
      // 订阅实例
  ...
  async beforeClose() {
    ...
    if (this.app.nacosClient) {
      try {
        const {
          isRegisterInstance,
          client: { serviceName, groupName, ip, port, metadata = {} },
        } = this.app.config.nacos;
        if (isRegisterInstance && serviceName && ip && port) {
          ...
          await this.app.nacosClient.deregisterInstance(
            serviceName,
            { ip, port, metadata },
            groupName
          );
